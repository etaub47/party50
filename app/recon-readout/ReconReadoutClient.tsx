        ...
        return (
            <div className="p-8 bg-black min-h-screen font-mono flex flex-col items-center justify-center text-center">
                <div className="border border-red-500 bg-red-950/20 rounded-xl p-8 max-w-md">
                    ...
                    <div className="text-[10px] text-red-300/80 uppercase border-t border-red-500/30 pt-4">
    ...
    return (
        <div className="p-6 bg-slate-950 min-h-screen text-slate-100 font-mono">
            ...
            <header className="mb-8 border-b border-green-500/30 pb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <Link href={"/"} className="text-xs text-slate-300 hover:text-white tracking-widest uppercase mb-2 inline-block">
                        ← BACK TO FIELD
                    </Link>
                    <h1 className="text-2xl font-bold text-green-400 tracking-wider">RECON READOUT</h1>
                    <p className="text-xs text-slate-200 mt-1">REAL-TIME SYSTEM STATUS SYNCHRONIZATION</p>
                </div>
                ...
                <div className="bg-slate-900 border border-green-500/40 px-6 py-3 rounded-lg flex items-center gap-4">
                    <span className="text-xs text-slate-200">HIDDEN ASSETS RETRIEVED:</span>
                    <span className="text-xl font-bold text-green-400">
                        {foundCount} <span className="text-xs text-slate-300">/ 10</span>
                    </span>
                </div>
            </header>

            {/* Mission Manifest Matrix Grid */}
            <h2 className="text-xs text-green-400 tracking-widest uppercase mb-4">Available Operations</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {missionStatuses.map((m) => (
                    <div
                        key={m.id}
                        className="bg-slate-900/60 p-4 border border-slate-800 rounded-lg flex justify-between items-center gap-3 transition-all hover:border-slate-700"
                    >
                        <div>
                            <h3 className="text-sm font-bold uppercase text-slate-50">{m.title}</h3>
                            <span className="text-[10px] text-slate-300 uppercase">{m.description}</span>
                            {(m.status === 'COMPLETED' || m.status === 'FAILED') && (
                                <div className="text-[10px] text-slate-300 uppercase mt-2 pt-2 border-t border-slate-800">
                                    <span className="text-slate-400">SQUAD: </span>
                                    {m.teammates.length > 0 ? m.teammates.join(', ') : 'SOLO RUN'}
                                </div>
                            )}
                        </div>
...
function StatusBadge({ status }: { status: MissionStatus['status'] }) {
    ...
    return (
        <span className={`text-[10px] px-3 py-1 font-bold rounded-md border uppercase tracking-wider shrink-0 whitespace-nowrap ${badges[status]}`}>
            {textLabels[status]}
        </span>
    );
}
